# 积分商城&直播机器人

⚠️ 本项目仅供学习交流使用，禁止用于商业或非法用途。

**本项目已经经由 Zread 解析完成，如果需要快速了解项目，可以点击此处进行查看：[了解本项目](https://zread.ai/zxc7563598/php-bilibili-danmu)**

---

## ✨ 核心功能

- **积分商城**: 观众可通过每日签到、开通大航海、赠送礼物等方式获取积分，并用于兑换虚拟道具或实体礼品。支持后台管理、上下架商品、积分变更记录等。

- **直播间打卡签到**: 观众每天可签到获取积分，并记录累计/连续签到天数，让直播间形成健康的活跃循环。

- **PK 播报**: PK 对战开启前，自动播报对手的活跃数据和贡献榜信息，支持自定义播报模板，让直播节奏更带感。

- **礼物答谢**: 收到礼物后自动播报感谢语，可设置金额门槛并自定义多条个性化文案随机展示。

- **定时广告**: 定时推送预设内容，支持多条文案随机发送，用于宣传加群、抽奖、活动、商单等。

- **进房欢迎**: 观众进入直播间时自动欢迎，支持多套话术随机展示，提升互动体验。

- **感谢关注**: 当用户关注直播间时自动感谢，并支持多条话术。

- **感谢分享**: 用户分享直播间时自动感谢，支持多条文案的随机展示。

- **自动回复**: 当弹幕触发预设关键词时自动回复，支持自定义多套回复方案，并随机输出，让机器人更自然不生硬。

- **自动禁言**: 与自动回复联动，对违规触发内容的观众进行临时禁言；并支持通过赠送指定金额的礼物自动解除禁言。

- **数据分析**: 记录直播间弹幕与礼物信息，生成用户每日消费统计、盲盒盈亏数据、弹幕词云等分析报告，帮助主播了解观众互动和贡献情况，更好地优化直播策略。


## 不想折腾服务器？试试免部署版本

如果你只是想跑个直播机器人，不想自己维护服务器、也不想折腾部署，可以看看同样由我开发的轻量本地版：

**[bilibili-live-assistant](https://github.com/zxc7563598/bilibili-live-assistant)** —— 跑在自己电脑上的 B 站直播机器人，下载安装包双击即可运行，无需服务器、无需环境、无需写代码，所有数据本地存储。

目前支持签到、进房欢迎、感谢关注/分享、礼物答谢、PK 播报、定时广告、关键词自动回复、自动禁言等核心互动功能，积分商城与数据分析将在后续版本加入。

- 想要完整的积分商城和数据看板 → 继续使用本项目
- 想要开箱即用、本地运行 → 前往 [bilibili-live-assistant](https://github.com/zxc7563598/bilibili-live-assistant)

## ✨ 部署方案

本项目提供多种部署方式，提供文字/视频教程，请前往[官方文档查看](https://hejunjie.life/danmusuite/introduction/about#%E9%83%A8%E7%BD%B2%E8%AF%B4%E6%98%8E)

## ✨ 预览？

有关前端预览内容，请参考对应的前端项目，本仓库不再重复提供相关展示。

[![Admin](https://img.shields.io/badge/vue--bilibili--danmu--admin-前端：管理后台-42b883?style=for-the-badge&logo=vue.js)](https://github.com/zxc7563598/vue-bilibili-danmu-admin)
[![Shop](https://img.shields.io/badge/vue--bilibili--danmu--shop-前端：移动端积分商城-3eaf7c?style=for-the-badge&logo=vue.js)](https://github.com/zxc7563598/vue-bilibili-danmu-shop)

## ✨ 配套项目

[![Core](https://img.shields.io/badge/php--bilibili--danmu--core-B站交互核心模块-blueviolet?style=for-the-badge&logo=php)](https://github.com/zxc7563598/php-bilibili-danmu-core)
[![Docker](https://img.shields.io/badge/php--bilibili--danmu--docker-Docker一键部署容器-2496ed?style=for-the-badge&logo=docker)](https://github.com/zxc7563598/php-bilibili-danmu-docker)
[![API](https://img.shields.io/badge/php--bilibili--danmu-项目本体-007acc?style=for-the-badge&logo=php)](https://github.com/zxc7563598/php-bilibili-danmu)
[![Admin](https://img.shields.io/badge/vue--bilibili--danmu--admin-前端：管理后台-42b883?style=for-the-badge&logo=vue.js)](https://github.com/zxc7563598/vue-bilibili-danmu-admin)
[![Shop](https://img.shields.io/badge/vue--bilibili--danmu--shop-前端：移动端积分商城-3eaf7c?style=for-the-badge&logo=vue.js)](https://github.com/zxc7563598/vue-bilibili-danmu-shop)
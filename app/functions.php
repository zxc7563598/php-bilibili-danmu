 ...
 * @return string
 */
function supplementStr($str = '', $number = 8): string
{
    if (!is_int($number) || strlen($str) > $number) {
        return $str;
    }
    $diff_number = $number - strlen($str);
    return str_repeat('0', $diff_number) . $str;
}
 ...
 *
 * @param string $str 字符串
 * @param integer $length 拆分后每个字符串长度
 * 
 * @return array
 */
function splitStr($str = '', $length = 1): array
{
